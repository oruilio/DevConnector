...
router.post('/', [auth, [
    ...
    // Build social object
    profileFields.social = {};          //initialize profileFields.social
    ...
    try {
        let profile = await Profile.findOne({ user: req.user.id });  //everytime we use a mongoose method, it's a promise, need to put an await in the front
        if (profile) {   //profile exists
          ...
          return res.json(profile);    //return ends the post
        }
...
router.get('/user/:user_id', async (req, res) => {
    try {
      ...
      console.error(err.message);
      if (err.kind == 'ObjectId') {    //check if it is an id (1 is also an objectid)
        return res.status(400).json({ msg: 'Profile not found' });
      }
      res.status(500).send('Server Error');
    }
